...
### 公開サイトのその他ページ
- プライバシーポリシー（`/privacy`）/ 運営者情報（`/about`）/ ブランド統一のカスタム404
- 悩み別LP・完了ページを新デザインに合わせて磨き込み、フッターを4カラムの導線に刷新
...
- AI施設マッチング（要件20・ルールベース）：相談者の条件（予算・エリア・要介護度・認知症・生活保護・医療対応・空室）から
  施設の適合度(%)と理由を算出し、案件詳細の「施設マッチング」タブに表示。ワンクリックで提案に追加。
- ユーザー管理（要件21）：管理者が社内スタッフの権限（管理者/相談員/閲覧者/広告担当）と有効状態を変更（`/admin/users`）。
- 連携設定（要件6・3）：管理者が電話番号・受付時間・LINE URL・GA/GTM/Google広告タグIDを管理画面から設定（`/admin/settings`）。
  計測タグは設定値→環境変数の順で公開サイトに自動出力。LINE/広告API/AI/LINE WORKS等の本連携は認証情報登録後に有効化。
